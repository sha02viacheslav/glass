      ...
      <Slider {...settingsTestimonials}>
        {GOOGLE_REVIEWS.map((item, index) => (
          <div key={index} className='item h-100'>
            <div className='testmon-box m-2'>
              <div className='d-flex justify-content-between'>
                <div className='avator'>
                  <p className='mb-0'>
                    <Link to={item.author_url} target='_blank' className=' purple-link'>
                ...
                <div className='d-flex flex-column align-items-end justify-content-between'>
                  <div className='star'>
                    ...
                    <span className='fs-16 text-gray ms-1'>{item.rating.toFixed(1)}</span>
                  </div>
                  <div className='fs-14'> {pastTime(item.time * 1000)}</div>
                </div>
              </div>
              <p className='fs-16 mt-3'>{item.text}</p>
            </div>
          </div>
        ))}
      </Slider>
