...
export const TimeSelection: React.FC<TimeSelectionProps> = ({
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: { xs: 'column', lg: 'row' }, gap: { xs: 10, lg: 24 } }}>
      <Box sx={{ flex: 1 }}>
        <Box>
          <Typography
            sx={{
        ...
        <Box sx={{ display: { xs: 'none', lg: 'block' } }}>{showCalendar && renderSelectedDate()}</Box>
      </Box>

      <Box
        sx={{
          display: { xs: showCalendar ? 'block' : 'none', lg: 'block' },
          opacity: { lg: showCalendar ? 1 : 0.2 },
          flex: 1,
        }}
      >
        <Box
                        ...
                        <td id={moment(element.date).format('MMMDD') + time.toString()} key={time}>
                          <Box
                            ...
                            sx={{
                              ...
                              background: isActive(element, time) ? 'var(--Light-Blue---Primary-000, #E8F0FE)' : '#fff',
                              display: 'flex',
                              alignItems: 'center',
                              margin: '4px 8px',
                              justifyContent: 'center',
